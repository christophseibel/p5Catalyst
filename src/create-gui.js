...
import * as controller from "./lib/controllers";

export function createGUI(generator) {
  // if (gui != undefined) gui.div.remove();

  gui.setLeft();

  // add logo up top (uses 'assets/generator-logo.svg', see style.css)
  let logo = gui.addField(gui.createField("logo", ""));
  ...
  //     "Download PNG",
  //     (controller) => {
  //       gui.p.save(Generator.getOutputFileName() + ".png");
  //     },
  //     (controller) => {
  //       controller._doUpdateChangeSet = false;
  //     }
  ...
  // 			gui.p.print(ffmpegExportSettings);
  // 			if (!isCapturingFrames) {
  // 				startCapture();
  // 				controller.controllerElement.html(
  ...
  // 	gui.createField('vidLoad', '')
  // );
  // guiVideoLoadingDiv.div.hide();
  // let loaderDiv = gui.p.createDiv();
  ...
  // gui.addController(
  // 	new controller.JSONFileLoader(
  // 		gui,
  // 		'jsonFileLoaderSettings',
  // 		'LANG_LOAD_SETTINGS',
  // 		(controller, file) => {
  // 			gui.p.print(controller, file);
  ...
  // gui.addController(
  // 	new controller.Button(gui, 'buttonHelpMe', 'LANG_HELP', controller => {
  // 		helpMe();
  // 	})
  // );
...
export function getAPaperResolutionOptionAtDpi(
  aNumber,
  dpi,
  isPortrait = true
) {
  ...
  return (
    ...
    } @ ${dpi} DPI: ` + `${isPortrait ? wPx : hPx} x ${isPortrait ? hPx : wPx}`
  );
}
